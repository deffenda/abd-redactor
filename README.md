...
- `src/document_manager_web.py` - local web app for AI detection, redaction download, and document summary PDF generation
...
- Entry point: `src/document_manager_web.py`
...
```bash
source .venv/bin/activate
uvicorn document_manager_web:app --app-dir src --reload --port 8000
```
...
- **Web API / Local Paths**: Immediate AI detection, redaction download, and document summary generation for manual uploads (`src/document_manager_web.py`); `process_files.py` remains a fast redaction-engine test path
